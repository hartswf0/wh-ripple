...
AUTHOR_REGEX = r"Loom Mason|Watson Hartsoe|watson\.hartsoe@gmail\.com|user@example\.com"
...
for repo_dir in sorted(REPOS.iterdir()):
    ...
    for line in touched.splitlines():
        ...
        if skip_path(line):
            continue
        if Path(line).suffix.lower() not in CODE_EXTS:
            continue
        counts[line] += 1
